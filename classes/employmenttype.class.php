...
class employmenttype {
	 ...
	 * @var Array
	 */
	private $fields =array('employmenttype_id', 'employmenttype', 'create_date', 'modify_date', 'delete_date');
	
	/**
	 * Array of feilds require information when validating 
	 * @var Array|null
	 */
	private $fields_required =array('employmenttype');
	
	/**
	 * Array of feilds and there types that are check when validating 
	 * @var Array|null
	 */
	private $fields_validation_type = array ('employmenttype_id'=>'INT', 'employmenttype'=>'TEXT', 'create_date'=>'TEXT', 'modify_date'=>'TEXT', 'delete_date'=>'TEXT');
	...
	Private function lists($orderby=NULL, $direction='ASC', $filter=NULL){
		...
		$sql = "SELECT employmenttype_id,
employmenttype,
create_date,
modify_date,
delete_date FROM employmenttype WHERE (delete_date ='00-00-0000 00:00:00' OR delete_date IS NULL)";
	...
	Private function create($source){
			try{
				...
				foreach($source['employmenttype'] AS $key=>$val){
					$field[] = $key;
					$value[] = ":".$key;
				}

				$sql = "INSERT INTO employmenttype (".implode(', ',$field).") VALUES (".implode(', ',$value).");";
...
create_date,
modify_date,
delete_date FROM employmenttype WHERE employmenttype_id = ". $id ." AND (delete_date ='00-00-0000 00:00:00' OR delete_date IS NULL)" ;
	...
	Private function update($source, $id){
			try{
				...
				$sql = "UPDATE employmenttype SET ".implode(', ',$field)." WHERE employmenttype_id =". $id;
	...
	Private function remove($id){
			...
			$sql = "UPDATE employmenttype SET delete_date=NOW() WHERE employmenttype_id =". $id;
	 ...
	 * 
	 * <pre>This Method will produce a list of all the element corresponding to the result of Employmenttype
	...
	public function getEmploymenttypeList($type='TABLE',$orderby=NULL, $direction='ASC', $filter=NULL){
		...
		switch(strtoupper($type)){
			...
			case 'AJAX' : $this->table->setRowsOnly(); 
						  $this->table->setIdentifier('employmenttype_id');
						  $this->table->setIdentifierPage('employmenttype');
...
'delete_date'=>'TEXT'));
				...
				$this->template->content(Box($this->table->genterateDisplayTable($result),'Employmenttype List', 'Shows the current listings for the Employmenttype. To create a new Listing <a href="employmenttype.php?action=create">Click Here</a>'));
	...
	Public function showEmploymenttypeDetails($id){
		...
		//if($this->checkAdminLevel(1)){
			$this->template->assign('FUNCTION', "<div class=\"button\" onclick=\"location.href='employmenttype.php?action=edit&id=".$id."'\">Edit</div>");
		//}
	...
	Public function editEmploymenttypeDetails($id){
		...
		$this->template->page('employmenttype.tpl.html');
		$this->template->assign('FORM-HEADER', '<form action="employmenttype.php?action=update&id='.$id.'" method="POST" name="'.$name.'">');
		
		$this->templateEmploymenttypeLayout($fieldMember, true);
		
		$this->template->assign('FUNCTION', "<input class=\"button\" type=\"image\" value=\"Update\"><div class=\"button\" onclick=\"location.href='employmenttype.php?action=show&id=".$id."'\">Cancel</div>");
	...
	Public function createEmploymenttypeDetails(){
		...
		$this->template->page('employmenttype.tpl.html');
		$this->template->assign('FORM-HEADER', '<form action="employmenttype.php?action=save" method="POST" name="'.$name.'">');
		
		$this->templateEmploymenttypeLayout('', true);
		
		$this->template->assign('FUNCTION', "<input class=\"button\" type=\"image\" value=\"Save\"><div class=\"button\" onclick=\"location.href='employmenttype.php?action=list'\">Cancel</div>");
	...
	Public function saveEmploymenttypeDetails(){
		...
		if ($this->Validate($_REQUEST)){
				...
				//if($this->admin->checkAdminLevel(1)){
					$this->template->assign('FUNCTION', "<input class=\"button\" type=\"image\" value=\"Update\"><div class=\"button\" onclick=\"location.href='employmenttype.php\">Cancel</div>");
				//}
				$this->template->assign('FORM-FOOTER', '</form>');
	...
	private function templateEmploymenttypeLayout($fieldMember, $input = false, $inputArray=array() ){
				...
				$id = @$fieldMember['employmenttype_id'];
								...
								@$this->template->assign('employmenttype_id', ($input)? $this->template->input('text', 'employmenttype_id', $fieldMember['employmenttype_id']):$fieldMember['employmenttype_id']);
				@$this->template->assign('employmenttype', ($input)? $this->template->input('text', 'employmenttype', $fieldMember['employmenttype']):$fieldMember['employmenttype']);
				...
				/*if(isset($id)){
					$this->template->assign('COMMENTS', $this->comment->getCommentBox($id, 'employmenttype'));
				}*/
