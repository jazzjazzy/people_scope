...
class radio{
	...
	function display($lable ,$field, $qid){
		...
		foreach($field AS $checkboxDetails){	
			$file .= '<input type="radio" name="q['.$qid.']" value="'.$checkboxDetails['multi_id'].'">'.$checkboxDetails['label'].'<br>';
		}
	...
	function getValues($fields=NULL, $postion="AFTER", $horizontal=false ){
		...
		foreach($_SESSION['Question_Details']['values'] AS $key=>$value){
			...
			$html .= "<div$float>";
			if($postion == "AFTER"){
				$html .= "<input type=\"radio\" name=\"".$value['value']."\" id=\"".$value['value']."\"><label for=\"".$value['value']."\">".$value['value']."</label>\n";
			}else{
				$html .= "<label for=\"".$value['value']."\">".$value['value']."</label><input type=\"radio\" name=\"".$value['value']."\" id=\"".$value['value']."\">\n";
			}
			$html .= "</div>";
		}
		}
			...
			jQuery.fn.exampleVal = function(){
				...
				$(".selection-list>li").each(function(index) {
					...
					if($("input[name=position]:checked").val() == "AFTER"){
						select += "<div><span><label for=\""+name+"\">"+name+"</label></span><span><input type=\"radio\" name=\""+name+"\" id=\""+name+"\"></span></div><div />";
					}else{
						select += "<span><input type=\"radio\" name=\""+name+"\" id=\""+name+"\"></span><span><label for=\""+name+"\">"+name+"</label></span><div />";
					}
				});
